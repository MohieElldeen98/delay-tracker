...
const AdminPanel = ({ currentUser }: { currentUser: User }) => {
    ...
    // متغيرات جديدة للتحكم في كلمات المرور
    ...
    // دالة حفظ الباسورد الجديد
    const handleSavePassword = async (userId: string) => {
        if (!newPassword.trim()) return alert("كلمة المرور لا يمكن أن تكون فارغة");
        try {
        ...
        <div className="space-y-4">
            ...
            {loading ? <div className="text-center py-8"><Loader2 className="w-8 h-8 animate-spin mx-auto text-orange-500" /></div> : (
                <div className="grid gap-3">
                    ...
                    {allUsers.map(u => (
                        <div key={u.id} className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col gap-3 shadow-sm">
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        {/* عرض كلمة المرور */}
                                        <div className="text-xs text-slate-500 flex items-center gap-2 mt-1" dir="ltr">
                                            <button onClick={() => togglePassword(u.id)} className="text-teal-600 hover:text-teal-800">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => { setEditingPasswordId(u.id); setNewPassword(u.password); }} className="p-2 bg-blue-50 text-blue-500 rounded-lg hover:bg-blue-100 transition" title="تغيير كلمة المرور">
                            ...
                            {/* فورم تعديل كلمة المرور (يظهر عند الضغط على زر التعديل) */}
                            {editingPasswordId === u.id && (
                                <div className="flex gap-2 mt-2 pt-3 border-t border-slate-100">
                                    <input 
                                        ...
                                        className="flex-1 p-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-blue-500 text-center"
                                        dir="ltr"
                                    />
                                    <button onClick={() => handleSavePassword(u.id)} className="px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-lg hover:bg-blue-700">
                                        حفظ
                                    </button>
                                    <button onClick={() => setEditingPasswordId(null)} className="px-4 py-2 bg-slate-100 text-slate-600 text-sm font-bold rounded-lg hover:bg-slate-200">
                                        إلغاء
                                    </button>
                                </div>
                            )}
